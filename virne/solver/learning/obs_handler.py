...
class ObservationHandler:
    ...
    def get_v_node_aggr_link_demands(self, v_net, v_node_id, aggr='sum', link_attr_benchmarks=None):
        l_attrs = v_net.get_link_attrs('resource')
        link_demands = []
        for l_attr in l_attrs:
            link_demand = [v_net.links[(n, v_node_id)][l_attr.name] for n in v_net.adj[v_node_id]]
    ...
    def get_p_nodes_status(self, p_net, v_net, node_slots, v_node_id=None):
    ...
    def get_v_nodes_status(self, v_net, node_slots, v_node_id=None, consist_decision=True, neighbor_flags=False):
