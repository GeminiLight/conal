...
class RLSolver(Solver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        ...
        self.if_allow_baseline_unsafe_solve = kwargs.get('if_allow_baseline_unsafe_solve', False)
        if self.use_baseline_solver:
            ...
            if baselin_solver_name == 'grc':
                ...
                if hasattr(self.baseline_solver, 'eval'):
    ...
    def log(self, info, update_time, only_tb=False):
        ...
        if self.verbose >= 1:
            if update_time ==0 or update_time % 1000 == 0:
                info_key_str = ' & '.join([f'{k}' for k, v in info.items() if sum([s in k for s in ['loss', 'prob', 'return', 'penalty', 'value']])])
                print(f'             {update_time:06d} | ' + info_key_str)
            info_str = ' & '.join([f'{v:+3.4f}' for k, v in info.items() if sum([s in k for s in ['loss', 'prob', 'return', 'penalty', 'value']])])
    ...
    def evaluate_actions(self, old_observations, old_actions, return_others=False):
        ...
        policy_dist = candidate_dist if self.mask_actions and self.maskable_policy else dist
