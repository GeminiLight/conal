...
class SafeRLSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        ...
        super(SafeRLSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
...
class FixedPenaltyPPOSolver(PPOSolver, SafeRLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        super(FixedPenaltyPPOSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
...
class NeuralLagrangianPPOSolver(PPOSolver, SafeRLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        super(NeuralLagrangianPPOSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
...
class RobustNeuralLagrangianPPOSolver(PPOSolver, SafeRLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        super(RobustNeuralLagrangianPPOSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
        self.baseline_solver = self.to_sub_solver()
        if self.use_baseline_solver == True:
...
class OldNeuralLagrangianPPOSolver(PPOSolver, SafeRLSolver):
    ...
    def update(self, avg_cost):
        ...
        for i in range(sample_times):
            ...
            if (self.update_time+1) % (self.repeat_times) == 0:
                ...
                print(f'cost_budget: {cost_budgets.mean().item():.4f}, budget_scaler: {final_safe_budget_scaler.mean().item():.4f},  mean_task_difficuties: {task_difficuties.mean().item():.4f}, mean_returns: {returns.mean().item():.4f}, mean_cost_returns: {cost_returns.mean().item():.4f}')
                # print(f'budget_scaler: {final_safe_budget_scaler.mean().item():.4f},  mean_task_difficuties: {task_difficuties.mean().item():.4f}, mean_returns: {returns.mean().item():.4f}, mean_cost_returns: {cost_returns.mean().item():.4f}')
...
class RewardCPOSolver(PPOSolver, SafeRLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        super(RewardCPOSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
        self.baseline_solver = self.to_sub_solver()
        self.use_baseline_solver = True
        self.baseline_solver.policy.eval()
        self.baseline_solver.searcher = get_searcher('greedy', 
...
class AdaptiveStateWiseSafePPOSolver(PPOSolver, SafeRLSolver):
    ...
    def __init__(self, controller, recorder, counter, make_policy, obs_as_tensor, **kwargs):
        super(AdaptiveStateWiseSafePPOSolver, self).__init__(controller, recorder, counter, make_policy, obs_as_tensor, **kwargs)
        self.srl_alpha = kwargs.get('srl_alpha', 1.)
        self.srl_beta = kwargs.get('srl_beta', 0.1)
