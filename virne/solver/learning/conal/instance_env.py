...
class InstanceRLEnv(JointPRStepInstanceRLEnv):
    ...
    def _get_p_net_obs(self, ):
        ...
        p_node_link_min_resource = self.obs_handler.get_link_aggr_attrs_obs(self.p_net, link_attr_types=attr_type_list, aggr='min', link_attr_benchmarks=self.link_attr_benchmarks)
        ...
        p_node_link_data = np.concatenate((p_node_link_min_resource, p_node_link_max_resource, p_node_link_mean_resource, p_node_link_sum_resource), axis=-1)
        ...
        v_num_to_routed_links = np.ones((self.p_net.num_nodes, 1), dtype=np.float32) * self.obs_handler.get_v_node_neighbor_flags(self.v_net, self.solution['node_slots'], self.curr_v_node_id).sum() / 10
        avg_distance = self.obs_handler.get_average_distance(self.p_net, self.solution['node_slots'], normalization=True)
        node_data = np.concatenate((node_data, v_node_sizes, v_num_to_routed_links, p_nodes_status, self.p_net_node_degrees, avg_distance), axis=-1)
    ...
    def _get_v_net_obs(self):
        ...
        v_num_to_routed_links = np.ones((self.v_net.num_nodes, 1), dtype=np.float32) * self.obs_handler.get_v_node_neighbor_flags(self.v_net, self.solution['node_slots'], self.curr_v_node_id).sum() / 10
        ...
        v_node_status = self.obs_handler.get_v_nodes_status(self.v_net, self.solution['node_slots'], self.curr_v_node_id, consist_decision=True, neighbor_flags=True)
        v_node_link_min_resource = self.obs_handler.get_link_aggr_attrs_obs(self.v_net, link_attr_types=['resource'], aggr='min', link_attr_benchmarks=self.link_attr_benchmarks)
        ...
        v_node_link_data = np.concatenate((v_node_link_min_resource, v_node_link_max_resource, v_node_link_sum_resource, v_node_link_mean_resource), axis=-1)
        node_data = np.concatenate((node_data, v_node_sizes, v_num_to_routed_links, v_node_status, v_node_link_data), axis=-1)
        # node_data = np.concatenate((node_data, v_node_sizes, v_num_to_routed_links, v_node_status), axis=-1)
...
class CostAwareInstanceRLEnv(InstanceRLEnv):

    def __init__(self, p_net, v_net, controller, recorder, counter, **kwargs):
        super(CostAwareInstanceRLEnv, self).__init__(p_net, v_net, controller, recorder, counter, **kwargs)
...
class SimpleRewardInstanceRLEnv(InstanceRLEnv):

    def __init__(self, p_net, v_net, controller, recorder, counter, **kwargs):
        super(SimpleRewardInstanceRLEnv, self).__init__(p_net, v_net, controller, recorder, counter, **kwargs)
