...
class InstanceRLEnv(JointPRStepInstanceRLEnv):
    ...
    def _get_p_net_obs(self, ):
        ...
        v_node_min_link_demend = self.obs_handler.get_v_node_aggr_link_demands(self.v_net, self.curr_v_node_id, aggr='min')
        # node data
        ...
        # node_data = np.concatenate((node_data, p_nodes_status, v2p_node_link_demand, p_node_degree, p_node_link_sum_resource, p_node_average_distance), axis=-1)
...
class InstanceRLEnvWithNrmRank(InstanceRLEnv):

    def __init__(self, p_net, v_net, controller, recorder, counter, **kwargs):
        super(InstanceRLEnvWithNrmRank, self).__init__(p_net, v_net, controller, recorder, counter, **kwargs)
        self.node_ranking_method = 'nrm'


class InstanceRLEnvWithNeaRank(InstanceRLEnv):

    def __init__(self, p_net, v_net, controller, recorder, counter, **kwargs):
        super(InstanceRLEnvWithNeaRank, self).__init__(p_net, v_net, controller, recorder, counter, **kwargs)
        self.node_ranking_method = 'nea'