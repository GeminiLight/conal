...
class Controller:
    ...
    def unsafely_place(
        ...
        if check_result:
            ...
            violation_value = -min(all_satisfiability_value_list)
        else:
            all_satisfiability_value_list = flatten_recurrent_dict(check_info)
        ...
        solution['v_net_violation'] += np.clip(violation_value, 0, None)
        solution['v_net_place_violation'] += np.clip(violation_value, 0, None)
        solution['v_net_single_step_violation'] = -100.
        solution['v_net_max_single_step_positive_violation'] = max(solution['v_net_max_single_step_positive_violation'], solution['v_net_single_step_violation'])
        return True, check_info
    ...
    def unsafely_route(
        ...
        if len(shortest_paths) == 0:
            solution['v_net_single_step_violation'] = 0.
            return False, check_info
        check_result_list = []
        check_info_list = []
        ...
        for p_path in shortest_paths:
            ...
            if check_result:
                ...
                violation_value = -min(all_satisfiability_value_list)
                solution['v_net_single_step_violation'] = violation_value
                return True, check_info
        # check_info_dict = {p_path: {p_link: {result}}}
        for check_info in check_info_list:
            all_satisfiability_value_list = list(flatten_recurrent_dict(check_info))
            if max(all_satisfiability_value_list) >= 0:
                violation_value = -min(all_satisfiability_value_list)
            else:
        ...
        for p_link in p_links:
            ...
            solution['link_paths_info'][(v_link, p_link)] = used_link_resources
        solution['v_net_violation'] += np.clip(best_violation_value, 0, None)
        solution['v_net_route_violation'] += np.clip(best_violation_value, 0, None)
        solution['v_net_single_step_violation'] = best_violation_value
        solution['v_net_max_single_step_positive_violation'] = max(solution['v_net_max_single_step_positive_violation'], solution['v_net_single_step_violation'])
        return True, check_info
    ...
    def unsafely_place_and_route(
        ...
        place_violation_value = solution['v_net_single_step_violation']
        # Route
        route_info = {l_attr.name: 0. for l_attr in v_net.get_link_attrs()}
        to_route_v_links = []
        v_node_id_neighbors = list(v_net.adj[v_node_id])
        ...
        for v_link in to_route_v_links:
            ...
            route_result, route_info = self.unsafely_route(v_net, p_net, v_link, (p_node_id, n_p_node_id), solution, 
                                        shortest_method=shortest_method, k=k)
            route_violation_value = solution['v_net_single_step_violation']
            route_violation_value_list.append(route_violation_value)
        route_violation_value_of_all_paths = sum(route_violation_value_list) if len(route_violation_value_list) > 0 else -100.
        # best_violation_value = place_violation_value + sum(route_violation_value_list)
        best_violation_value = max(place_violation_value, route_violation_value_of_all_paths)
        solution['v_net_single_step_violation'] = best_violation_value
        solution['v_net_max_single_step_positive_violation'] = max(solution['v_net_max_single_step_positive_violation'], solution['v_net_single_step_violation'])
        # print(solution['v_net_single_step_violation'], place_violation_value, route_violation_value_list)
        return True, route_info
    ...
    def unsafely_link_mapping(
        ...
        for v_link_pair in sorted_v_links:
            ...
            violation_value = solution['v_net_single_step_violation']
