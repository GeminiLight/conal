...
topology="wxn"                    # Topology name. Options: [wx500]
...
declare -A pretrained_model_path_dict
use_pretrained_model=1
pretrained_model_path_dict["a3c_gcn"]="PRETRAINED_MODEL_PATH"
pretrained_model_path_dict["ddpg_attention"]="PRETRAINED_MODEL_PATH"
pretrained_model_path_dict["pg_cnn"]="PRETRAINED_MODEL_PATH"
pretrained_model_path_dict["conal"]="PRETRAINED_MODEL_PATH"


if [ $topology == "wxn" -a $use_pretrained_model == 1 ]; then
    pretrained_model_path=${pretrained_model_path_dict[$pretrained_model_name]}
else
    pretrained_model_path="null"
fi

if [ $topology == "wxn" ]; then
    # wx100 topology
    if [ $num_train_epochs == "0" ]; then
        # inference setting
        aver_arrival_rate_list=$(seq $aver_arrival_rate 0.02 $aver_arrival_rate)
        identifier="-test-$pretrained_model_name"
        echo $pretrained_model_path
    else
        # pretrain setting
        aver_arrival_rate_list=($aver_arrival_rate)
        pretrained_model_path="null"
    fi
else
...
p_net_num_nodes_list=$(seq 1000 200 1000)
for p_net_num_nodes_var in $p_net_num_nodes_list
do
    echo p_net_num_nodes $p_net_num_nodes_var
    p_net_num_nodes=$p_net_num_nodes_var
    ...
    do
        ...
        do 
            ...
            python main.py \
                ...
                --lr_actor=0.001 \
                --lr_critic=0.001 \
                --summary_dir="exp_data/conal/time_scale" \
                --seed=$seed \
                --summary_file_name="time_scale_$p_net_num_nodes_$topology-$solver_name-k_shortest_$k_shortest-aver_arrival_rate_$aver_arrival_rate$identifier.csv"
        done
    done
done